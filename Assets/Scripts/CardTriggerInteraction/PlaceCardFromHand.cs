...
public class PlaceCardFromHand : NetworkBehaviour, IDropHandler
{
    ...
    {//we must put a condition that a card can be dropped only where the tile is eligible
        ...
        if (gameManager.GetComponent<GameManager>().IsRunningPlayer() && placeManager.GetCardSelectedFromHand() != null) //bisogna mettere molte più condizioni per mettere la carta
